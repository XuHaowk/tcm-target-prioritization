...
def create_negative_samples(validated_pairs, all_compounds, all_targets, ratio=1.0, random_seed=42):
    """
    创建负样本对（假定未验证的对是负样本）
    
    参数:
        validated_pairs: 验证的化合物-靶点对（阳性样本）
        all_compounds: 所有化合物ID列表
        all_targets: 所有靶点ID列表
        ratio: 负样本与阳性样本的比例
        random_seed: 随机种子
    ...
    # 创建已验证对的集合（用于快速查找）
    validated_set = set((pair[0], pair[1]) for pair in validated_pairs)
    
    # 计算需要的负样本数量
    num_negative = int(len(validated_pairs) * ratio)
    
    # 生成负样本
    negative_pairs = []
    attempts = 0
    max_attempts = num_negative * 10  # 设置最大尝试次数避免无限循环
    
    while len(negative_pairs) < num_negative and attempts < max_attempts:
        # 随机选择化合物和靶点
        comp_id = np.random.choice(all_compounds)
        target_id = np.random.choice(all_targets)
        
        # 确保这个对不在已验证集合中
        if (comp_id, target_id) not in validated_set:
            negative_pairs.append((comp_id, target_id, 0))  # 标签0表示负样本
            validated_set.add((comp_id, target_id))  # 添加到集合中避免重复
        
        attempts += 1
    
    # 为阳性样本添加标签1
    positive_pairs = [(pair[0], pair[1], 1) for pair in validated_pairs]
    
    # 合并所有样本
    all_labeled_pairs = positive_pairs + negative_pairs
    
    print(f"创建了 {len(negative_pairs)} 负样本对")
